...
export abstract class CommandMessage {
  ...
  protected async getChannelMessage(message: ChannelMessage) {
    try {
      ...
      this.logger.debug(
        `Getting message details - clan: ${clanId}, channel: ${message.channel_id}, message: ${message.message_id}`,
      );
      ...
      if ((this.client as any).clans && clanId && clanId !== '0') {
        ...
        if (!clan) {
          this.logger.warn(
            `[AUTO-FETCH] Clan ${clanId} chưa có trong cache, tiến hành fetch lại...`,
          );
          try {
            ...
            this.logger.error(
              `[AUTO-FETCH] Không thể fetch clan ${clanId}: ${err.message}`,
            );
          ...
          try {
            ...
            if ((this.client as any).clans) {
              ...
              if (clan) {
                ...
                if (channel) {
                  this.logger.debug(
                    `Successfully fetched channel ${message.channel_id} from clan ${clanId}`,
                  );
                  ...
                  if (
                    ...
                    this.logger.debug(
                      `Using channel.messages.create() for channel ${message.channel_id}`,
                    );
                    return await channel.messages.create(
                      this.filterReplyOptions(options),
                    );
                  }

                  if ((channel.messages as any).send) {
                    this.logger.debug(
                      `Using channel.messages.send() for channel ${message.channel_id}`,
                    );
                  ...
                  if ((channel as any).send) {
                    this.logger.debug(
                      `Using channel.send() for channel ${message.channel_id}`,
                    );
                  ...
                  if ((channel as any).createMessage) {
                    this.logger.debug(
                      `Using channel.createMessage() for channel ${message.channel_id}`,
                    );
                    return await (channel as any).createMessage(options);
                  }

                  this.logger.warn(
                    `No suitable message creation method found for channel ${message.channel_id}`,
                  );
                } else {
                  this.logger.warn(
                    `Channel not found in clan: ${message.channel_id}`,
                  );
                }
              } else {
            ...
            if ((this.client as any).sendMessage) {
              this.logger.debug(
                `Using direct sendMessage to ${message.channel_id}`,
              );
              return await (this.client as any).sendMessage(
                message.channel_id,
                options,
              );
            }
            ...
            if ((this.client as any).createMessage) {
              this.logger.debug(
                `Using createMessage for ${message.channel_id}`,
              );
              return await (this.client as any).createMessage(
                message.channel_id,
                options,
              );
            }
            ...
            if ((this.client as any).getChannel) {
              this.logger.debug(
                `Trying to get channel ${message.channel_id} directly`,
              );
              const channel = await (this.client as any).getChannel(
                message.channel_id,
              );
              if (channel) {
                this.logger.debug(
                  `Successfully got channel ${message.channel_id} directly`,
                );
                if ((channel as any).send) {
                  return await (channel as any).send(options);
                ...
                this.logger.warn(
                  `Failed to get channel ${message.channel_id} directly`,
                );
              }
            }
      ...
      this.logger.error(
        `Error in getChannelMessage: ${error.message}`,
        error.stack,
      );
      return null;
    }
